...
// This is the Iris dataset in CSV format, hard-coded for this example.
// See the csv import version in polars-cli lib.rs for reading from a file.
...
mod tests {
    ...
    #[test]
    fn test_calculate_filters_rows() {
        let df = calculate(5.0).unwrap();
        assert!(df.shape().0 > 0);
        assert!(df.get_column_names().contains(&"species".to_string()));
    }
}
